export default function componentFourWayMovement(k) {
    function fourWayMovement(speed = 100, { arrows = true, keyboard = 'wasd' } = {}) {
        const keys = {
            up: [],
            left: [],
            down: [],
        ...
        return {
            ...
            update() {
                ...
                if (!this.movementDir.isZero()) {
                    const vel = this.movementDir.unit().scale(this.movementSpeed) // unit() to fix diagonal movement
                    this.move(vel)
                }
            },
        }
